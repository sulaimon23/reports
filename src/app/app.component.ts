...
@Component({
  selector: 'app-root',
  standalone: true,
  imports: [MatSidenavModule, CommonModule, MatIconModule, RouterOutlet],
  ...
  animations: [
    ...
    trigger('textFade', [
      ...
      transition('hidden => visible', [animate('300ms 150ms ease-out')]),
    ]),
...
export class AppComponent {
  ...
  readonly mobileDrawerState = computed(() => (this.opened() ? 'open' : 'closed'));
  ...
  getNavLinkClasses(isActive: boolean): string {
    const baseClasses =
      'nav-link w-full px-3 py-2.5 border-0 bg-transparent flex items-center cursor-pointer transition-all duration-200 rounded-lg text-left text-gray-700 font-normal text-sm leading-6 relative group';
  ...
  getSidebarClasses(): string {
    const baseClasses =
      'bg-white shadow-lg rounded-xl flex flex-col transition-all duration-300 relative z-10 h-screen overflow-hidden';
    const mobileClasses = this.isMobile() ? 'fixed left-0 z-[1002]' : '';
